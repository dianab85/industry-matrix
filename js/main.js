...
function getCoord(height, width){
    ...
    width = Math.ceil(strokeWidth/2) + width;

    //coordinates
    coord = [
        [
            [Math.ceil(strokeWidth/2),height*3],
            [Math.ceil(strokeWidth/2),height*2],
            [Math.ceil(strokeWidth/2),height],
            [Math.ceil(strokeWidth/2),Math.ceil(strokeWidth/2)]
        ],
        [
            [width,height*3],
            [width,height*2],
            [width,height],
            [width,Math.ceil(strokeWidth/2)]
        ],
        [
            [width*2,height*3],
            [width*2,height*2],
            [width*2,height],
            [width*2,Math.ceil(strokeWidth/2)]
        ],
        [
            [width*3,height*3],
            [width*3,height*2],
            [width*3,height],
            [width*3,Math.ceil(strokeWidth/2)]
        ],
        [
            [width*4+strokeWidth,height*3],
            [width*4+strokeWidth,height*2],
            [width*4+strokeWidth,height],
            [width*4+strokeWidth,Math.ceil(strokeWidth/2)]
        ]
    ];
    return coord; 
...
function drawLine(colX,colY,string,val){    
    ...
    line.animate({'x2':coord[colY][$xVal[0]][0] ,'y2':coord[colY][$xVal[0]][1]}, 250);
}
...
function drawGraph(tabsObj,snapObj){
    ...
    for (let i = 0; i < tabs; i++) {
        ...
        for (let ii = 0; ii < currentTabCount; ii++) {

            var currentVal = [currentTabArray[ii],ii];

            if(currentTabArray[ii] !== null && currentTabArray[ii+1] !== null){

                var line = snapObj.line(
                    //x1,y1
                    coord[ii][currentVal[0]][0],
                    coord[ii][currentVal[0]][1]+shift,
                    //x2, y2
                    coord[ii+1][currentTabArray[ii+1]][0],
                    coord[ii+1][currentTabArray[ii+1]][1]+shift
                );
                ...
                line.animate({'x2':coord[ii+1][currentTabArray[ii+1]][0] ,'y2':coord[ii+1][currentTabArray[ii+1]][1]+shift}, 250);
            }
        }
        shift = shift + 4;
    }
...
function createGraph() {
    ...
    var selectedValues = Object.values(tabs[selectedTab]);
...
$(window).resize(function () {
    ...
    if($svgCount > 1){
        ...
        for (let i = 0; i < $svgNodeArray.length; i++) {
...
$('.btn-submit').click(function() {
    ...
    $.ajax({
        ...
        data: {submission: JSON.stringify(tabs), interactive_name: interactive_name},
...
if (interactive_name === null || interactive_name === "") {
    $('.container').html("You must specify an interactive name. Contact Denise Rigato (denise.rigato@keypathedu.com) for help.");
}
...
function showResults() {
    ...
    $.ajax({
        method: "GET",
        url: "db/interactive_data.php",
        data: {interactive_name: interactive_name},
        success: function (data) {
    ...
    $('.btn-activity.btn-compare').click(function() {
        ...
        drawGraph(JSON.parse(Cookies.get(interactive_name)),canvas);

        // set drawing events back to original graph so tabs work on that one only
        getCoord($('#industry-matrix tbody td:nth-child(2)').height(), $('#industry-matrix tbody td:nth-child(2)').width());
